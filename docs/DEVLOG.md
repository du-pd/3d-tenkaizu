...
## 完了（複数ページ印刷の修正, PR #6）
- [x] 複数パーツが1枚に収まらない場合、プレビューは複数ページに分割される（従来通り）。
- [x] **印刷が1枚に潰れるバグを修正**: `.panel.p2d { overflow:hidden }` が印刷時に解除されず
      ページ分割が抑制されていた。印刷CSSで overflow/height/display を是正し、各ページを
      `break-after: page` で別シートに。末尾の空白ページも防止。
- [x] `@page` の用紙サイズを選択中の用紙(A4/A3/カスタム)に合わせて JS で注入。従来は A4 固定で、
      A3/カスタム印刷が実寸で出ない不具合があった。印刷時 svg は width:100% を無効化し intrinsic な
      mm サイズで出力（実寸保証）。
- [x] テスト: 複数パーツ→複数ページ分割（io）、印刷PDFのシート数=プレビュー枚数（ui）。
...
- PR #6: 複数ページ印刷の修正（印刷が1枚に潰れる不具合 / @page用紙サイズ対応）+ テスト（merged）
